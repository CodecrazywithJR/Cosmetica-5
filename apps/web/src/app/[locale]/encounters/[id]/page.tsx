...
import { useEncounter, useAddTreatment, useFinalizeEncounter, useUpdateEncounter } from '@/lib/hooks/use-encounters';
...
import { useTreatments, type Treatment } from '@/lib/hooks/use-treatments';
...
export default function EncounterDetailPage() {
  ...
  const handleGenerateProposal = async () => {
    try {
      ...
      router.push(routes.proposals.detail(locale, result.proposal_id));
    } catch (err) {
  ...
  return (
    <AppLayout>
      <RBACGuard roles={[ROLES.ADMIN, ROLES.PRACTITIONER]}>
        <div>
          {/* Encounter Header */}
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginTop: 12 }}>
                  {encounter.clinical_photos && encounter.clinical_photos.length > 0 ? (
                    encounter.clinical_photos.map((photo: any) => {
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginTop: 12 }}>
                  {encounter.documents && encounter.documents.length > 0 ? (
                    encounter.documents.map((doc: any) => (
          ...
          <div className="card mb-4">
            ...
            <div className="card-body">
              {encounter.status === 'finalized' && encounter.charge_proposal ? (
                <div>
                  <div style={{ marginBottom: 8 }}>
                    <span className={`badge badge-${encounter.charge_proposal.status}`}>
                      {t(`clinical:chargeProposal.status.${encounter.charge_proposal.status}`, encounter.charge_proposal.status)}
                    </span>
                  </div>
                  <div style={{ marginBottom: 8 }}>
            ...
            <div className="card-body" style={{ padding: 0 }}>
              {encounter.encounter_treatments && encounter.encounter_treatments.length > 0 ? (
                <table className="table">
                  ...
                  <tbody>
                    {encounter.encounter_treatments.map((et: any) => (
                      <tr key={et.id}>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const selected = (treatments as Treatment[]).find((tr) => tr.id === e.target.value);
                        if (selected && selected.default_price) {
                          setUnitPrice(selected.default_price);
                        }
                      }}
                      required
                    >
                      <option value="">{t('clinical:modals.treatmentIdPlaceholder')}</option>
                      {(treatments as Treatment[]).map((tr) => (
                        <option key={tr.id} value={tr.id}>
                          {tr.name} — {currencyFormatter.format(Number(tr.default_price))}
                        </option>
                      ))}
                    </select>
                  </div>
